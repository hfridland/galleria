...
import { getCurInd } from 'features/status-select'
import { pictIndexes } from '../features/status-slide'
import { setPictInd as setPictIndDisp } from '../features/status-slide'
import {
  isPrevEnabled as isPrevEnabledDisp,
  isNextEnabled as isNextEnabledDisp,
} from 'features/status-select'
...
const Detail = () => {
  ...
  const setPictInd = (ind: number) => dispatch(setPictIndDisp(ind))
  ...
  }${pictDescr.images.hero.small.substring(1)}`
  const pathArtist = `${
    process.env.PUBLIC_URL
  }${pictDescr.artist.image.substring(1)}`
  const pathContent = `${
    process.env.PUBLIC_URL
  }${pictDescr.images.gallery.substring(1)}`
      ...
      <div className="detail">
        <div className="detail-info">
          <div className="detail-info__pict-names">
            <img
              src={pathPict}
              alt={pictDescr.name}
              className="detail-info__pict-names__picture"
            />
            <div className="detail-info__pict-names__names__text">
              <div className="detail-info__pict-names__names__text__title">
                {pictDescr.name}
              </div>
              <div className="detail-info__pict-names__names__text__artist-name">
                {pictDescr.artist.name}
              </div>
            </div>
            <img
              src={pathArtist}
              alt="artist"
              className="detail-info__pict-names__artist"
            />
          </div>
          <div className="detail-info__description">
            <div className="detail-info__description__year">
              {pictDescr.year}
            </div>
            <p className="detail-info__description__text">
              {pictDescr.description}
            </p>
            <div
              className="detail-info__description__show-src-btn"
              onClick={() => setShowPictView(true)}
            >
              GO TO SOURCE
            </div>
          </div>
        ...
        <div className="detail-buttons">
          <div className="detail-buttons__texts">
            <div className="detail-buttons__texts__pict-title">
              {pictDescr.name}
            </div>
            <div className="detail-buttons__texts__artist-name">
              {pictDescr.artist.name}
            </div>
          </div>
      ...
      {showPictView && (
        <div className="pict-view">
          <div className="pict-view__content">
            <div
              className="pict-view__content__close-btn"
              onClick={() => setShowPictView(false)}
            >
              CLOSE
            </div>
            <img
              src={pathContent}
              alt="gallery"
              className="pict-view__content__image"
            />
          </div>
        </div>
      )}
